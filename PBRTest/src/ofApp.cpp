...
void ofApp::setup()
{
    ...
    m_cameraParams.filmISO = 100.0f;
    m_cameraParams.shutterSpeed = 1.0f / 60.0f;
    m_cameraParams.aperture = 0.5f;
    m_cameraParams.exposureValue = lb::CalcEVFromCameraSettings( m_cameraParams.aperture, m_cameraParams.shutterSpeed );
...
{  
    ...
    m_cubeMap.LoadDDSTexture( "textures/totonga_radiance.dds" );
    lb::CheckGLError(); // clear GLEW errors
}
    ...
    switch ( m_appMode )
    {
        case AppMode::NORMAL_VIEW:
        {
            m_camera.begin();
            {
                ...
                m_shader.begin();
               //     m_shader.setUniform1i( "uCubeMap", 2 );
                    DrawScene();
                m_shader.end();
             }
            m_camera.end();
