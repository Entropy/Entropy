...
{
    float linearDepth = ( -_vertexZ - projectionInfo.nearClip ) / ( projectionInfo.farClip - projectionInfo.nearClip );
    int slice = int( max( linearDepth * NUM_SLICES_Z, 0.0f ) );

    // TODO: tile size is 64, SHR by 6 to find x,y index
    ivec3 cluster_coord;
    cluster_coord.xy = ivec2( _coord_xy * projectionInfo.rcpViewportDims * ivec2( NUM_SLICES_X, NUM_SLICES_Y ) );
