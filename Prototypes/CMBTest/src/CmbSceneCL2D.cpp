...
namespace ent
{
	...
	void CmbSceneCL2D::setup()
	{
		...
		for (int i = 0; i < 3; ++i) {
			m_clImages[i].initWithTexture(m_dimensions.x, m_dimensions.y, GL_RGBA32F);
		}
	...
	void CmbSceneCL2D::addDrop()
	{
		...
		m_dropKernel->setArg(1, ofDefaultVec2(ofRandom(m_dimensions.x), ofRandom(m_dimensions.y)));
